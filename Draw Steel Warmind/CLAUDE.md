...
Re-audited 2026-07-07 against upstream main 5f6a04b (508 new commits since the then-current branch base 289c2bc). Seven seam auditors verified every row below against the new source. Verdict: NO Warmind code was broken by upstream; all corrections are folded into the rows and Lessons below. The 2026-06-10 note that MainAttackerForTarget was deleted is superseded -- it is restored (see Casting and Squad strikes rows). Rebase executed 2026-07-11 onto upstream main ac12e06 -- the merge-tree prediction held for the audited leg (only .gitignore conflicted through 5f6a04b; root CLAUDE.md auto-merged); see Rebase status under process notes for the ac12e06 leg and post-rebase verification.
...
| Ability behaviors and condition traits (Stage 2, 2026-07-11) | `ability:try_get("behaviors", {})` (field can be absent; engine reads it the same way, `ActivatedAbility.lua` ~819); entries are ActivatedAbilityBehavior game types discriminated by `.typeName`. Condition inflict: base `ActivatedAbilityBehavior:ConditionID()` returns nil (~3788, safe on every behavior type); `ActivatedAbilityApplyOngoingEffectBehavior:ConditionID()` (~4379) resolves `behavior.ongoingEffect` through `dmhub.GetTable("characterOngoingEffects")` and returns that effect's `.condition` -- a CONDITION id whose display name lives in `dmhub.GetTable("charConditions")[id].name` (the name-space `HasNamedCondition` lowercase-matches, `Creature.lua` ~10954). Aid-attack ongoing-effect guid `e234f1f4-9953-43bd-894c-d96adbb63f84` = `Warmind.Traits.AID_ATTACK_EFFECT_GUID`. Forced movement: NEVER call `ability:ForcedMovementType()`/`IsForcedMovement()` on enumerated abilities -- the DS override (`MCDMActivatedAbility.lua` ~2561) requires `try_get("invoker")`, never set on top-level clones, so they return nil/false for everything; detect via the raw `ability:try_get("forcedMovement")` field ("push"\|"pull"\|"slide"\|"vertical_*", authored in the ability editor) or a `ActivatedAbilityRelocateCreatureBehavior` in behaviors on a CREATURE-targeting ability (on self-space targetTypes -- self/emptyspace/emptyspacefriend/anyspace, the engine's own GetIcon idiom ~353 -- a Relocate moves the caster: Charge/Disengage/Move Speed, not forced movement). `ability:GetRadius(casterCreature\|nil, castingSymbols\|nil)` is nil-safe (numeric radius returns directly; formula radius with nil caster degrades to leading-number parse or 0; ~944, DS override ~3787 only adds `castingSymbols.abilityRadiusBonus`). Raw static fields: `.numTargets` (GoblinScript string, class default "1", ~749; live value needs a token via GetNumTargets), `.resourceCost` (default "none", ~250) -- static malice check `resourceCost == CharacterResource.maliceResourceId` mirrors the engine's own MaliceCost GoblinScript field (`MCDMActivatedAbility.lua` ~3475). `creature:ActiveOngoingEffects()` -> array of effect instances with `.ongoingEffectid` (`Creature.lua` ~6751); `creature:IsConcealed()` (`MCDMCreature.lua` ~3200, reads token.hasConcealment only) | `ActivatedAbility.lua`, `MCDMActivatedAbility.lua`, `Creature.lua`, `MCDMCreature.lua`, `Condition.lua` ~21 |
| Casting | `ActivatedAbilityInvokeAbilityBehavior.ExecuteInvoke(invokerToken, abilityClone, casterToken, "inherit", symbols, options)` with `options.targets`, `options.countsAsCast = true`, `symbols.mode = 1`; completion signaled via `ability.OnFinishCast` (chain the existing one; the engine extracts it via try_get before casting and fires it once through an idempotent finishHandler). **ExecuteInvoke BLOCKS internally until the cast completes or is cancelled** (entry wait + `while not finishedCasting` with `castCount <= 1` cancel break) -- see Lessons learned. Signature re-verified 2026-07-07 and post-rebase 2026-07-11 (now ~line 469). CORRECTION: `MainAttackerForTarget` is RESTORED upstream (`ActivatedAbilityCast.lua` ~662) -- the editor-driven Cast path sources invoked sub-abilities AND caster-benefit behaviors per-target from the main attacker (first targetPairs entry whose b == target charid, falling back to caster); Warmind's direct ExecuteInvoke call is unaffected, but targetPairs ordering is load-bearing for squads (see Lesson 13). New hardening 2026-07: when the prompt callback resolves targeting, an `aiResolvedTargeting` flag makes the engine bypass the action-bar invokeAbility UI even for RequiresPromptWhenCast abilities (removes a class of AI-cast hangs); the inherit branch now locks `symbols.allowedtargets` to exactly options.targets, so always populate options.targets for inherit invokes | `DMHub Game Rules/AbilityInvokeAbility.lua` (~469) |
| Prompt interception | `token.properties._tmp_aicontrol` (counter, class default 0) + `_tmp_aipromptCallback` (default false), both `Creature.lua` ~198. Engine consults at `AbilityInvokeAbility.lua` ~595 (re-verified 2026-07-07 and post-rebase 2026-07-11; consult logic byte-identical, only moved): callback(invoker, caster, abilityClone, symbols, options) -> "inherit" (use options.targets) \| "prompt" (manual UI) \| "skip". Write directly (NO ModifyProperties -- `_tmp_` is transient, baseline does the same) | `AbilityInvokeAbility.lua`, `Creature.lua` |
...
3. The file watcher only fires for IN-PLACE writes (cp); the Write/Edit tools write atomically
   (temp file + rename) and the watcher misses them (Lesson 17). After Write/Edit deploys,
   `POST /reload` with a JSON body (`-d '{}'` -- body-less gets "411 Length Required") reloads
   all mods in one call. `GET /status` reports filesChanged / changedMods.
4. Confirm the reload in Player.log (`RefreshLocalFiles for Warmind`; on a watcher-triggered
   reload the chain starts with `Local file changed`) AND a `debug.getinfo` fingerprint
   (Lesson 15). Confirm zero new console errors in the same tail.
5. Verify the store copy matches the repo byte-for-byte (`diff -q`) after deploying.
...
16. **The standard maneuvers carry their effects in rule text, not top-level structured fields** (live-probed 2026-07-11 on Delian Tomb goblins). Knockback's push lives in power-roll tier rules (behaviors: CustomTrigger + PowerRoll only) -> `forcedMovementType` reads nil. Grab's grabbed and Hide's hidden ride in `ActivatedAbilityDrawSteelCommandBehavior.rule` strings ("grabbed", "hidden (eoe)") parsed by the engine's own rule grammar (`MCDMAbilityBehavior.lua` ExecuteCommandInternal ~1504) -> `appliesGrabbed`/`isHide` read false on the standard maneuvers. This is the designed degrade-to-nil, but it means the Stage 2 D-specs (forced_movement, grab, hide) will NOT light up on the standard maneuvers via traits alone -- the specs session must decide: match the standard maneuvers by canonical ability name (they are engine-standard, names are stable, baseline did the same), or read the DrawSteelCommand `rule` field for the exact literals "grabbed"/"hidden" (single-token match only; do NOT grow a rule-text parser -- that grammar belongs to the engine). Bespoke monster abilities that inflict conditions via ApplyOngoingEffect behaviors DO classify (e.g. Swamp Stink -> conds=weakened, live-verified), as do malice costs and aid attack.
17. **The mod-store file watcher is writer-SENSITIVE, not writer-agnostic** (observed 2026-07-11 evening, correcting the same-day Lesson 14): with monitoring AND checkout confirmed on (`code.monitorid` set, `m.checkedout=true`, probed via the bridge at the time), deploying via the Claude Write/Edit tool produced NO `Local file changed` line and `GET /status` stayed `filesChanged:false` -- those tools write atomically (temp file + rename), which the watcher misses; the earlier same-day session used `cp` (in-place write) and the watcher fired. Reliable recipe: Write/Edit the store files, then `POST /reload` with a JSON body (`-d '{}'` -- a body-less POST gets "411 Length Required"); it reloads all mods in one call. Confirm with the `RefreshLocalFiles for Warmind` log line plus a `debug.getinfo` fingerprint (Lesson 15; observed Traits.Get 68 -> 173 -> 197 across the two Stage 2 deploys).
...
| 2 | Generic competence: full Traits, generic spec set (signature strike, charge, area-if-N, grab, knockback, aid attack, hide, reposition), Scoring tactic biases, Prompts (Shift smart, Push/Pull/Slide with real shape), Squads port (re-path fix + no-signature fallback). Squad `targetPairs` contract re-verified intact post-rebase and again 2026-07-07 (see Squad strikes row) -- the baseline port plan stands. Detailed implementation plan: `STAGE2_PLAN.md` (written 2026-07-07 from the 508-commit upstream audit + baseline port inventory + Book Two ability census) | IN PROGRESS -- Workstreams A/C/B landed 2026-07-11: Traits expansion (forcedMovementType, inflictsConditions, appliesGrabbed, isHide, isAidAttack, aoeRadius, raw numTargets, costsMalice), snapshot per-enemy `aidAttacked` map, `Scoring.FindReachableConcealment` + flanking/aid_attack/high_ground tactics. L0 clean, L1 green (30/30), adversarially reviewed (findings fixed), L3 clean (deployed, reloaded, fingerprinted, probed on live goblins -- see Lessons 16/17 for the data-shape findings). Remaining: D (spec set), E (prompt handlers), F (squad port), then P2.5 closure + L4 |
...
- Rebase status (2026-07-11): REBASED. Branch base is now upstream main ac12e06 (was 289c2bc). Audit coverage caveat: the 2026-07-07 seven-auditor audit covers through 5f6a04b only; the 21 further commits (5f6a04b..ac12e06) got a targeted spot-check, not a full audit. Spot-check findings: seam logic byte-identical -- ExecuteInvoke now ~469 and the prompt consult ~595 (both shifted +15 by a remote-invoke deferral upstream added in the editor-driven Cast path ~265, which Warmind's direct ExecuteInvoke does not use); Creature.lua changes are movement rules only (new SafeFallDistance, CostToClimb rework, CanClimb -> IsClimber); ActivatedAbilityCast.lua untouched (MainAttackerForTarget still ~662); upstream also deleted compendium/reference/ and added the `data` submodule. Conflicts resolved during the rebase: `.gitignore` (kept upstream's rewritten block plus the branch's `*.DS_Stor*` line; dropped the obsolete `!compendium/reference/`) and root CLAUDE.md (upstream moved the main-copy path to `D:\dev\dmhub\draw-steel-codex\`; kept upstream's line with the branch's Working Rules section below it -- root CLAUDE.md also carries upstream's Crows/Crowdex section and the `cond()`-is-not-a-ternary warning that Warmind scoring code should heed). Post-rebase verification: luac -p passed on all 13 Warmind Lua files, every Warmind file ASCII-clean, both load-bearing seams re-verified. origin/tiny-monster-ai still holds the pre-rebase history, so the next push needs --force-with-lease.
