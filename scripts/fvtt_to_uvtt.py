...
that the Draw Steel Codex / DMHub UVTT importer can consume.

Strict wall classification: only patterns with faithful UVTT or DMHub
extension handling are recognized
    door=1                              -> portal(closed=True)
    door=0, dir!=0                      -> foundry_unrecognized_walls
    door=0/2, threshold.{light,sight}!=None
            and light!=move and sight!=move
                                        -> portal(closed=False)  (true window)
    door=2                              -> portal(closed=True, secret=True)
    door=0, sight==20, move==20         -> line_of_sight (solid wall)
    door=0, sight==10, move==20         -> foundry_terrain_walls
    door=0, sight==0,  move==20         -> foundry_invisible_walls

Terrain walls, invisible walls, and unrecognized walls are emitted into
custom top-level fields ("foundry_terrain_walls", "foundry_invisible_walls",
and "foundry_unrecognized_walls"). DMHub offers per-category import behavior
controls for these extensions. Other UVTT consumers ignore unknown fields.

Pillow is used for image conversion and downscaling. If Pillow is not
available the image is embedded as-is only when its dimensions can be
inspected and it is already within DMHub's 8192px extraction limit.

Usage:
    python3 fvtt_to_uvtt.py SCENE.json [--foundry-data PATH] [-o OUT.dd2vtt]
                                       [--image PATH] [--no-embed-image]
                                       [--max-image-size 8192]
                                       [--no-secret-extension] [-v]
"""
...
def resolve_image_src(src: str, foundry_data: Optional[Path],
    ...
    if is_remote_src(src):
        warn(f"Image is a remote URL ({src}); skipping image embed.")
        return None
    ...
    if warn_missing:
        warn(f"Could not locate image file '{urllib.parse.unquote(src)}'. "
...
def scene_background_src(scene: Dict[str, Any]) -> Tuple[Optional[str], str]:
    ...
    Foundry V11+ keeps the background image under levels[0].background.src.
    Older formats may have a top-level "background" or "img" key.
    """
...
def resolve_image_path(scene: Dict[str, Any], foundry_data: Optional[Path],
    ...
    if src:
        ...
        if tile is not None:
            warn("Primary background image could not be resolved; trying the "
                 "largest map-sized tile image instead.")
    ...
    if tile is not None:
        ...
        if isinstance(src, str):
            img_path = resolve_image_src(src, foundry_data, scene_path)
            if img_path is not None:
                warn("Scene has no background.src/img; using the largest "
                     f"map-sized tile image instead ({urllib.parse.unquote(src)}).")
                return ResolvedImage(
                    path=img_path,
    ...
    if isinstance(thumb, str) and thumb:
        warn("Scene has no background.src/img and only a thumbnail reference "
             f"was found ({urllib.parse.unquote(thumb)}); writing UVTT without "
             "an embedded map image. Pass --image PATH to choose the full map.")
    else:
        warn("Scene has no background image reference; writing UVTT without "
             "an embedded map image. Pass --image PATH to choose the full map.")
    return None
...
def encode_image(img_path: Path, max_size: int,
                 ...
                 verbose: bool) -> Tuple[Optional[str], int, int]:
    """Return (base64_png, final_width_px, final_height_px).

    Resizes the source image to (target_w_px, target_h_px) -- the Foundry
    scene's declared image dimensions -- before optionally downscaling to
    max_size. Returning the FINAL pixel size lets the caller adjust
    pixels_per_grid so wall coords (in grid units) line up with the
    rendered image (regardless of how aggressively we downsampled).

    When Pillow is missing, the file's bytes pass through unmodified only if
    we can prove the image is already within the requested size cap.
    """
    if not HAVE_PIL:
        dims = read_image_dimensions_without_pillow(img_path)
        if dims is None:
            raise ImageEncodeError(
                "Pillow is not installed and the image dimensions could not "
                "be inspected. Install Pillow so the converter can resize the "
                "image safely."
            )
        ...
        if max(source_w, source_h) > max_size:
            raise ImageEncodeError(
                f"Pillow is not installed and the image is {source_w}x{source_h}, "
                f"which exceeds the {max_size}px limit. Install Pillow so the "
                "converter can downscale it before embedding."
            )

        target_w = max(1, int(round(target_w_px)))
        target_h = max(1, int(round(target_h_px)))
        if (source_w, source_h) != (target_w, target_h):
            warn("Pillow is not installed; embedding image bytes as-is without "
                 "resizing to Foundry scene dimensions. Install Pillow if walls "
                 "do not align with the map image.")
        else:
            warn("Pillow is not installed; embedding image bytes as-is because "
                 f"the image is already within the {max_size}px limit.")
        with open(img_path, "rb") as f:
    ...
    with Image.open(img_path) as im:
        if im.mode not in ("RGB", "RGBA"):
            im = im.convert("RGBA")
        sw, sh = im.size
        log(verbose, f"Image source size: {sw}x{sh}")
        ...
        if (sw, sh) != (target_w, target_h):
            log(verbose, f"Resizing to scene dimensions {target_w}x{target_h}")
        ...
        if long_edge > max_size:
            ...
            log(verbose, f"Downscaling to {new_size[0]}x{new_size[1]} "
        ...
        data = buf.getvalue()
        log(verbose, f"Encoded PNG size: {len(data) / (1024 * 1024):.2f} MB")
...
def classify_wall(w: Dict[str, Any]) -> str:
    """Return one of: 'door', 'secret_door', 'window', 'wall', 'terrain',
    'invisible', 'unrecognized'.

    'terrain' means a partial-sight blocker that still blocks movement (for
    example a stove or large furniture). UVTT has no equivalent primitive, so
    these are emitted as DMHub extension data and imported with the selected
    terrain wall material.

    'invisible' means a wall that blocks movement but not vision. These cannot
    be represented by UVTT line_of_sight without inventing vision blockers, so
    they are emitted as a DMHub extension and imported with invisible wall
    material.
    """
    door = wall_int(w, "door", 0)
    sight = wall_int(w, "sight", 20)
...
def convert_walls(scene: Dict[str, Any], grid_size: float,
                  pad_x_px: float, pad_y_px: float, verbose: bool
                             ...
                             List[Dict[str, Any]],
                             Dict[str, Any]]:
    """Return (line_of_sight, portals, terrain_walls, invisible_walls,
    unrecognized_walls, report).

    Wall coordinates in Foundry are stored in canvas-space (which includes
    the scene's padding region). `pad_x_px` / `pad_y_px` bundle the full
    image-origin offset (padding * size + shiftX/Y + textures.offsetX/Y),
    so subtracting them puts us in image-space pixels. Then divide by
    `grid_size` to land in UVTT grid units.
    """
...
def convert_lights(scene: Dict[str, Any], grid_size: float,
    ...
    for l in lights_in:
        ...
        lx = as_float(l.get("x"), 0.0) or 0.0
        ly = as_float(l.get("y"), 0.0) or 0.0
        out.append({
            "position": {
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if args.max_image_size <= 0:
        print("ERROR: --max-image-size must be greater than 0", file=sys.stderr)
        return 2
    if args.max_image_size > DMHUB_MAX_IMAGE_SIZE:
        warn(f"--max-image-size {args.max_image_size} exceeds DMHub's "
             f"{DMHUB_MAX_IMAGE_SIZE}px image extraction limit; clamping to "
             f"{DMHUB_MAX_IMAGE_SIZE}.")
    ...
    if isinstance(grid, dict):
        ...
        warn("No grid info in scene; defaulting to 100 px per grid.")
        grid_size = 100.0
    if grid_size <= 0:
        warn("Invalid grid size in scene; defaulting to 100 px per grid.")
        grid_size = 100.0
    ...
    if abs(tex_rotation) > 0.01:
        warn(f"Scene has levels[0].textures.rotation = {tex_rotation}; this "
             f"converter does not undo image rotation. Clear the rotation "
             f"in Foundry before exporting for accurate alignment.")
    if abs(tex_scale_x - 1.0) > 0.01 or abs(tex_scale_y - 1.0) > 0.01:
        warn(f"Scene has levels[0].textures.scaleX/Y = {tex_scale_x}, "
             f"{tex_scale_y}. The converter resizes the embedded image to "
             f"the scene's full canvas, so non-default texture scale may "
             f"cause walls to misalign. Set scaleX/Y back to 1 in Foundry "
             f"if this happens.")
    if tex_fit and tex_fit != "fill":
        warn(f"Scene has levels[0].textures.fit = '{tex_fit}'. The "
             f"converter assumes fit='fill' (image stretched to scene). "
             f"Other fit modes may cause walls to misalign; switch to "
             f"'fill' in Foundry if alignment is off.")
    ...
    log(args.verbose, f"Grid size: {grid_size} px per grid")
    log(args.verbose, f"Scene size: {scene_w_px}x{scene_h_px} px")
    log(args.verbose, f"Padding: {padding}; shiftX: {shift_x}; shiftY: {shift_y}")
    log(args.verbose, f"textures.offsetX/Y: {tex_offset_x}, {tex_offset_y}; "
                      f"fit={tex_fit}; scaleX/Y={tex_scale_x}, {tex_scale_y}")
    log(args.verbose, f"Image-origin offset: ({pad_x_px}, {pad_y_px}) px")
    ...
    if args.no_embed_image:
        ...
        if tile is not None:
            ...
            if tile_h is not None:
                map_h_px = tile_h
            log(args.verbose, f"Using scene tile placement without embedding: "
        ...
        if resolved_image is not None:
            ...
            log(args.verbose, f"Image source: {resolved_image.source}")
            log(args.verbose, f"Image path: {resolved_image.path}")
            log(args.verbose, f"Resolved image origin: ({pad_x_px}, {pad_y_px}) px")
            log(args.verbose, f"Resolved map size: {map_w_px}x{map_h_px} px")
            try:
                image_b64, image_w_px, image_h_px = encode_image(
                    resolved_image.path, args.max_image_size,
                    map_w_px, map_h_px, args.verbose)
            except ImageEncodeError as e:
                print(f"ERROR: {e}", file=sys.stderr)
                return 2

    line_of_sight, portals, terrain_walls, invisible_walls, unrecognized, report = convert_walls(
        scene, grid_size, pad_x_px, pad_y_px, args.verbose)
    ...
    if image_w_px > 0:
        log(args.verbose, f"Image final size: {image_w_px}x{image_h_px} px "
                          f"-> pixels_per_grid="
    ...
    if size_mb > 50:
        warn(f"Output is larger than 50 MB; the DMHub importer may struggle. "
             f"Try --max-image-size 2048.")
